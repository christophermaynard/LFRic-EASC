...
\title{LFRic: The design and implementation of a Weather and Climate model}
...
Object Orientation to encode the rules of the API. \textcolor{red}{(Comment: Define what API means?)}
...
location of the degrees of freedom (hereafter dofs) for these spaces are shown in
...
computed a number of times per Picard iteration \textcolor{red}{(Comment: Explain/give reference to what is Picard iteration?)},
nominally 3 stages are used. 
...
code can be kept separate from the science code. As outlined in 
...
call. The permitted arguments to these invoke prodecures are restricted
...
of freedom (hereafter DoF) between columns (e.g. density defined on 
$Q_p^D$ function space, see Subsection~\ref{sec:sub:spatial} for more details) 
and the ones that do (e.g. velocity defined on $RT_0$ space, 
...
is the call to an {\em invoke} procedure which does not exist in the 
source code and is instead generated by PSyclone. 
The invoke means {\em execute these kernels looping over the horizontal mesh}. 
PSyclone can paralellise this horizontal looping with MPI,
...
\textcolor{red}{(Comment: Perhaps explain that k refers to vertical?)};
