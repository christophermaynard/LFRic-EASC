...
Object Orientation to encode the rules of the application programming interface (hereafter API). 
...
presented in Section~\ref{sec:psyclone} and linear solvers and 
preconditioners are presented in Section~\ref{sec:Solver}.
...
maximum/minimum edge length \textcolor{red}{ratio} of $\approx 1.3$ achieved by using only
...
of freedom.
\textcolor{red}{(Comment: Dofs on lower two diagrams are not really visible.)}
}
\end{figure}
%
...
\textcolor{red}{(Comment: vertical-only velocity space is mentioned in 
Subsection~\ref{sec:sub:preconditioner}, however it is not defined here)}.
...
nominally 3 stages are used. \textcolor{red}{(Comment: Explain/give reference to what is Picard iteration?)}
...
\textcolor{red}{(Comment: The code listings can become broken over two pages. 
Is there a command to enforce keeping them together?)}
...
\caption{\label{fig:objects} Schematic diagram of the family of objects used
...
shorter. \textcolor{red}{(Comment: Reference to support this conclusion?)}
...
checks the state of the halos and if \textcolor{red}{(and only if)} they are `dirty' it calls the halo
...
different elements of the mesh (nodes, edges or faces) \textcolor{red}{(Comment: What about vertices?)}.
...
\textcolor{red}{(Comment: Advanced for F2003 or advanced regarding to older Fortran?)} This approach
...
Fig.~\ref{fig:class_hierarchy}, left): \textcolor{red}{(Comment: Are these plots actual UML diagrams?)}
\begin{itemize}
...
library. For horizontally discontinuous function spaces \textcolor{red}{(such as the
...
couplings within one vertical column. 
\textcolor{red}{(Comment: The vertical-only velocity space mentioned above is
not defined in Subsection~\ref{sec:sub:spatial})}.
Matrices of this type can be
