...
then in effect a DSEL compiler. PSyclone also does one other important
code generation function. In the algorithm layers, the calls to the
invoke procedure are replaced with a specific prodcedure call to the
PSy layer. The PSy layer code can then be optimised depending on the
kernels in the invoke and the metadata.
...
the data object, in this case fields and operators\footnote{An
  operator is a mapping between one space and another.}. Secondly, what is
the data acess pattern, {\em e.g.} \verb+GH_INC+ means the data is
...
As has been shown in figure~\ref{fig:psykal}, the PSyKAl layered
