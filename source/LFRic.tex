...
\begin{abstract}
  ...
  The paper describes the LFRic atmosphere model being developed by
  the Met Office to replace the Unified Model for its climate and
  weather operations during the 2020s. LFRic uses the GungHo dynamical
  core and runs on a semi-unstructured cubed-sphere mesh. The design
  follows object oriented principles to modularise functionality and
  better benefit from externally obtained packages where possible. In
  particular the design imposes a separation of concerns between the
  science code and parallel code to promote performance
  portability. An application called PSyclone, developed at the STFC
  Hartree centre can generate the parallel code enabling efficient
  deployment of a single science code onto different hardware. The
  paper provides an overview of the scientific requirement, the design
  of the LFRic software infrastructure, the operation of the PSyclone
  code generator, and experiences of writing and deploying the new
  model. Preliminary compute performance results of the GungHo
  dynamical core are described.
...
such as I/O and distributed memory communications.
...
In GungHo this semi-Lagrangian scheme is replaced by an Eulerian
finite volume 
...
typically expressed over MPI. Multi-core nodes present
additional opportunities and challenges to applications in terms of exploitation of
shared address space within a node. Heterogenous compute nodes such as
...
algorithm layer. Kernels are written to operate on a single vertical column of data. 
...
The API is enforced by using Fortran 2003 object
...
Section~\ref{sec:psyclone}. The generated code is Fortran.
...
then in effect a DSEL compiler.
...
kernel is general and can operate on any function spaces, hence, the
...
Note that while most of the LFRic infrastructure supports a
horizontally unstructured mesh, the partitioner is specialised to
provide partitioning only of cubed-sphere and planar meshes. If a
different mesh is required it is a relatively straightforward change
to extend the partitioner to support it.
...
halo exchanges as a
method of providing communication between partitions. However, the
following aspects of the data model within the LFRic infrastructure
make implementing distributed memory parallelism using halo exchanges
...
communication routeing tables required to perform a halo
exchange. These tables  
are then reused every time a halo exchange is required.

In order to generate the routeing table, each rank needs to supply the
...
routeing tables it generated were optimal (the halo exchanges called
...
by ESMF, but the time taken to initialise the routeing tables is much
shorter.
...
horizontally unstructured meshes and a FEM formulation, where variables are held on
...
nothing in the approach to prevent other languages being targeted in
...
In short, the design of LFRic has been developed and deployed
successfully so far. The GungHo dynamical core is routinely run in a
range of configurations. After initially being run in serial for many
months while the first version of PSyclone was developed, the
introduction of PSyclone enabled it to be run in parallel (both MPI
and OpenMP) on 220,000 cores of the Met Office Cray XC40 within less
than two weeks.
...
Currently, generation of MPI and OpenMP code is supported by PSyclone. The approach of
...
envisaged that in response to profiling an applicaion an HPC expert
...
element method on a range of different horizontally unstructured meshes. Should the
