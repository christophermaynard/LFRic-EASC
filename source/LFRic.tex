...
\caption{\label{fig:fem-spaces} Location of degrees of freedom at
  $p=0$, from top left to bottom right, $Q_{1}$, $N_0$, $RT_0$ and $Q_0^D$ finite element spaces.
...
%    \subfloat[]{\includegraphics[width=0.75\linewidth]{Fig8b_weak_scale.eps}}
    \caption{\label{fig:OMP_scale}a) Log-log plot of strong scaling of
      ...
      guide the eye.} 
%   b) Log-linear plot of weak scaling of 50
%      timesteps of the baroclinic wave problem for a fixed local
%      volume of $24x24$ cells with 30 levels. The meshes range from
%      C288 to C1152. The teal bars, labelled MPI, show MPI only, {\em
%        i.e} 36 MPI ranks per node. The navy bars, labelled OMP show
%      mixed-mode, with 6 MPI ranks per node and 6 OpenMP threads per rank.}
  \end{center}
\end{figure}
...
Currently a single transformation script for OpenMP is used on all
code. In the future, it is
