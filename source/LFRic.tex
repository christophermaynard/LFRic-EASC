...
three transformations need to be applied. The colouring transformation
...
have the OpenMP transformations applied. This script, shown in
Listing~\ref{lst:omp.py} in Appendix~\ref{sec:app1}, has 17 lines of
executable Python and is all that is required to apply OpenMP to the
whole model. Psyclone can also apply specialized directives for
different loops, as un-transformed loops are esentially nodes in
an invoke schedule which can be reached individually.

Applying the OpenMP script (Listing~\ref{lst:omp.py}) without
the distributed memory support to the invoke call described in
...
and no distributed memory support},label={lst:PSy_omp}]
  ! Look-up colour map
...
\section*{\label{sec:app1}Appendix}
